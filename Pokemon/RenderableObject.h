...
public:
	...
	void setTexPos(float x1, float y1, float x2, float y2)	{ std::get<0>(m_textureRect) = x1; std::get<1>(m_textureRect) = y1; std::get<2>(m_textureRect) = y2 - y1; std::get<3>(m_textureRect) = x2 - x1; }
